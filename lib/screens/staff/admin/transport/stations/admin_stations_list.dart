...
class AdminStationsList extends StatelessWidget {
  ...
  Widget _buildCards() {
    return Column(
      children: page.results.map((station) {
        return Container(
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  AdminTransportStatusBadge(isActive: station.isActive),
                ],
              ),
...
class _Actions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StaffActionGroup(
      children: [
        IconButton(
          key: Key('admin-station-detail-${station.id}'),
          tooltip: 'Détail',
          onPressed: () => onOpenDetail(station),
          icon: const Icon(Icons.visibility_outlined),
        ),
        if (canManage) ...[
          IconButton(
            key: Key('admin-station-edit-${station.id}'),
            tooltip: 'Modifier',
          ...
          if (station.isActive)
            IconButton(
              key: Key('admin-station-deactivate-${station.id}'),
              tooltip: 'Désactiver',
              onPressed: () => onDeactivate(station),
              icon: const Icon(Icons.block, color: Color(0xFFB42318)),
            )
          else
            IconButton(
              key: Key('admin-station-activate-${station.id}'),
              tooltip: 'Activer',
              onPressed: () => onActivate(station),
              icon: const Icon(
